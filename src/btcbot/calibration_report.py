...
are not, so counts here are WINDOWS (the row at the tradeable time nearest ``at_tau_sec``), never raw rows.
...
def one_row_per_window(rows: Sequence[dict], *, at_tau_sec: float = 300.0, field: str = "p_model") -> list[dict]:
...
def build_report(rows: Sequence[dict], *, at_tau_sec: float = 300.0, field: str = "p_model", min_n: int = 10) -> dict:
    ...
    return {
        "windows": len(per), "field": field, "at_tau_sec": at_tau_sec,
...
def render(report: dict) -> str:
    def block(title: str, buckets: Sequence[Bucket]) -> list[str]:
        ...
        for b in buckets:
            ...
            lines.append(f"  {b.label:<19} {b.n:>7}  {mp:>6}  {rate:>8}  {b.ci95[0]:.0%}-{b.ci95[1]:.0%}"
                         f"{'':<6}{'small sample' if b.small else ''}")
        return lines
    out = [f"{report['windows']} settled windows, model field {report['field']}, read ~{report['at_tau_sec']:.0f}s before close"]
