...
class LogisticModel:
    ...
    from that dict is treated as its training-set mean (contributes 0 after standardization), not an error,
    so a caller need not thread every feature through every call site."""
    ...
    def predict_proba(self, features: Mapping[str, float]) -> float:
        ...
        for name, w, mean, scale in zip(self.feature_names, self.weights, self.mean, self.scale):
            x = (features.get(name, mean) - mean) / scale
            z += w * x
        return _sigmoid(z)
