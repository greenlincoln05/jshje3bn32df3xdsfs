...
everything built so far (Phases 2-4) has only ever been validated offline, against fakes and fixtures. This
...
## 3. Analyze what you recorded (offline again, no network needed)
...
profitability claims without recorded out-of-sample results.
...
`btcbot.model.log_prediction`). Nothing that runs automatically does that yet -- a live or paper strategy
loop doing so is Phase 5 -- so right now this will say "nothing to score" against a fresh recording. That is
expected, not a bug.

## 4. `auth-check` (optional -- needs your own credentials)
